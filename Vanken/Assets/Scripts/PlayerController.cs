...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetButton(attackButton) && !anim.GetCurrentAnimatorStateInfo(0).IsName("Kicce"))
        {
            anim.SetBool("isWalking", false);
            anim.SetTrigger("hit");
        }
    ...
    void Inputs()
    {
        ...
        bool tapDownDKey = Input.GetKeyDown(leftKey);
        bool tapDownAKey = Input.GetKeyDown(rightKey);
        
        bool holdDownDKey = Input.GetKey(leftKey);
        bool holdDownAKey = Input.GetKey(rightKey);
